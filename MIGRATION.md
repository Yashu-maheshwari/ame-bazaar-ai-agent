...
- **n8n Supabase PostgreSQL Ping Timeout Fix (`render.yaml` v3.3)**: Configured `DB_TYPE=postgresdb`, `DB_POSTGRESDB_SSL_REJECT_UNAUTHORIZED=false`, and `DB_POSTGRESDB_CONNECTION_TIMEOUT_MS=30000`. Resolved TypeORM background connection pool SSL verification handshake timeout on Supabase Transaction Pooler (port 6543) which previously caused `"Database ping failed (1): Database connection timed out"`.
- **Dynamic DATABASE_URL Entrypoint Mapper (`docker-entrypoint.sh` & `Dockerfile` v3.4)**: Created container boot wrapper `docker-entrypoint.sh` that dynamically parses single `DATABASE_URL` secret into n8n's required `DB_POSTGRESDB_HOST`, `DB_POSTGRESDB_PORT`, `DB_POSTGRESDB_DATABASE`, `DB_POSTGRESDB_USER`, `DB_POSTGRESDB_PASSWORD`, and `DB_POSTGRESDB_SSL_REJECT_UNAUTHORIZED=false` variables at runtime without hardcoding or manually setting 6 separate environment variables in Render.
